<?php
namespace Neos\Security\Authentication;
...
use Neos\Security\Log\SecurityLoggerInterface;
use Neos\Security\Authentication\Token\SessionlessTokenInterface;
use Neos\Security\Context;
use Neos\Security\Exception\NoTokensAuthenticatedException;
use Neos\Security\Exception\AuthenticationRequiredException;
use Neos\Security\Exception;
use Neos\Security\RequestPatternInterface;
use Neos\Security\RequestPatternResolver;
...
class AuthenticationProviderManager implements AuthenticationManagerInterface
{
     ...
     *
     * @return array Array of \Neos\Security\Authentication\AuthenticationProviderInterface
     */
        ...
        foreach ($providerConfigurations as $providerName => $providerConfiguration) {
            ...
            if (isset($providerConfiguration['entryPoint'])) {
                ...
                if (!class_exists($entryPointClassName)) {
                    $entryPointClassName = 'Neos\Security\Authentication\EntryPoint\\' . $entryPointClassName;
                }
                if (!class_exists($entryPointClassName)) {
                    throw new Exception\NoEntryPointFoundException('An entry point with the name: "' . $entryPointName . '" could not be resolved. Make sure it is a valid class name, either fully qualified or relative to Neos\Security\Authentication\EntryPoint!', 1236767282);
                }
