<?php
namespace Neos\Security\Authorization;
...
use Neos\Security\Exception\NoInterceptorFoundException;
...
class InterceptorResolver
{
    ...
    public function resolveInterceptorClass($name)
    {
        ...
        $resolvedObjectName = $this->objectManager->getCaseSensitiveObjectName('Neos\Security\Authorization\Interceptor\\' . $name);
        if ($resolvedObjectName !== false) {
            return $resolvedObjectName;
        }
